...
import {
  getProfileInquiryForEmail,
  updateProfileInquiryStatus,
} from "@/lib/data/promoter-inquiries";
import { notifyCustomerProfileInquiryStatus } from "@/lib/email/promoter-notifications";
...
export async function PATCH(request: Request) {
  ...
  try {
    await updateProfileInquiryStatus(
      inquiryId,
      auth.user!.id,
      status,
    ...
    if (notifyStatuses.includes(status)) {
      const inquiry = await getProfileInquiryForEmail(inquiryId);
      if (inquiry) {
        notifyCustomerProfileInquiryStatus({
          guestEmail: inquiry.guestEmail,
          guestName: inquiry.guestName,
          status,
          preferredEvent: inquiry.preferredEvent,
          promoterNotes:
