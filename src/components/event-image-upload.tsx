...
export function EventImageUpload({ ownerId, value, onChange }: EventImageUploadProps) {
  ...
  async function handleFile(file: File | null) {
    ...
    setError(null);
    try {
      const url = await uploadEventImage(file, ownerId);
      onChange(url);
    } catch (err) {
    ...
    <div className="space-y-3">
      ...
      {value ? (
        <div className="relative aspect-[21/9] max-h-48 w-full overflow-hidden rounded-lg border border-wtva-dark-300">
          <Image src={value} alt="Event preview" fill className="object-cover" unoptimized />
        </div>
      ) : null}
