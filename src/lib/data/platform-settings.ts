...
export async function getPlatformSettings(): Promise<PlatformSettings> {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "venue_submission_fee, venue_listing_months, event_submission_fee, event_ticket_commission_pct, vip_commission_pct, require_payment, auto_approve_venues, auto_approve_events, driver_listing_fee, driver_listing_months, driver_booking_commission_pct",
    )
    .eq("id", 1)
    .maybeSingle();
