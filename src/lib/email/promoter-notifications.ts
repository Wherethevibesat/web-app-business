...
export function notifyVenueOwnerPromoterLinkRequest(params: {
  ownerEmail: string;
  ownerName: string;
  promoterName: string;
  promoterEmail: string;
  ...
  const venueList = params.venueNames.map((v) => `<li>${v}</li>`).join("");
  const body = `<p><strong>${params.promoterName}</strong> (${params.promoterEmail}) requested to promote at:</p>
<ul>${venueList}</ul>
<p>Review and approve or reject in your business portal.</p>
<p><a href="${businessPortalUrl("/promoters")}" style="display:inline-block;margin-top:12px;padding:10px 16px;background:#111;color:#fff;text-decoration:none;border-radius:8px;font-weight:600">Review promoter requests</a></p>`;

  sendEmailSafe({
    to: params.ownerEmail,
    subject: `Promoter request: ${params.promoterName}`,
    html: layout(`Hi ${params.ownerName || "there"},`, body),
    text: `${params.promoterName} (${params.promoterEmail}) requested venue access: ${params.venueNames.join(", ")}. Review: ${businessPortalUrl("/promoters")}`,
  });
}

export function notifyAdminsPromoterLinkRequest(params: {
  adminEmails: string[];
  promoterName: string;
  promoterEmail: string;
  venueNames: string[];
  ...
  const venueList = params.venueNames.map((v) => `<li>${v}</li>`).join("");
  const body = `<p><strong>${params.promoterName}</strong> (${params.promoterEmail}) requested promoter access to:</p>
<ul>${venueList}</ul>
<p>You can also approve from the admin portal.</p>
<p><a href="${adminPortalUrl("/submissions?tab=promoters")}" style="display:inline-block;margin-top:12px;padding:10px 16px;background:#111;color:#fff;text-decoration:none;border-radius:8px;font-weight:600">Review in admin</a></p>`;

  sendEmailSafe({
    to: params.adminEmails,
    subject: `New promoter venue request: ${params.promoterName}`,
    html: layout("Admin notification", body),
    text: `${params.promoterName} requested access to: ${params.venueNames.join(", ")}. Review: ${adminPortalUrl("/submissions?tab=promoters")}`,
  });
}

export function notifyVenueOwnerPromoterEventRequest(params: {
  ownerEmail: string;
  ownerName: string;
  promoterName: string;
  promoterEmail: string;
  ...
  const body = `<p><strong>${params.promoterName}</strong> (${params.promoterEmail}) submitted a new event for your approval:</p>
<ul>
<li><strong>${params.eventTitle}</strong></li>
<li>Venue: ${params.venueName}</li>
<li>Starts: ${params.startsAt}</li>
</ul>
<p><a href="${businessPortalUrl("/promoters")}" style="display:inline-block;margin-top:12px;padding:10px 16px;background:#111;color:#fff;text-decoration:none;border-radius:8px;font-weight:600">Review promoter events</a></p>`;

  sendEmailSafe({
    to: params.ownerEmail,
    subject: `Promoter event pending: ${params.eventTitle}`,
    html: layout(`Hi ${params.ownerName || "there"},`, body),
    text: `${params.promoterName} submitted "${params.eventTitle}" at ${params.venueName} (${params.startsAt}). Review: ${businessPortalUrl("/promoters")}`,
  });
}

export function notifyAdminsPromoterEventRequest(params: {
  adminEmails: string[];
  promoterName: string;
  promoterEmail: string;
  eventTitle: string;
  ...
  const body = `<p><strong>${params.promoterName}</strong> (${params.promoterEmail}) submitted a promoter event:</p>
<ul>
<li><strong>${params.eventTitle}</strong></li>
<li>Venue: ${params.venueName}</li>
<li>Starts: ${params.startsAt}</li>
...
<p><a href="${adminPortalUrl("/submissions?tab=promoters")}" style="display:inline-block;margin-top:12px;padding:10px 16px;background:#111;color:#fff;text-decoration:none;border-radius:8px;font-weight:600">Review in admin</a></p>`;
  ...
  sendEmailSafe({
    to: params.adminEmails,
    subject: `New promoter event: ${params.eventTitle}`,
    html: layout("Admin notification", body),
    text: `${params.promoterName} submitted "${params.eventTitle}" at ${params.venueName}. Review: ${adminPortalUrl("/submissions?tab=promoters")}`,
  });
}
...
export function notifyCustomerProfileInquiryStatus(params: {
  guestEmail: string;
  guestName: string;
  status: string;
  preferredEvent?: string | null;
  ...
  let body = `<p>Your message to the promoter is now: <strong>${statusLabel}</strong>.</p>`;
  if (params.preferredEvent?.trim()) {
    body += `<p>Regarding: ${params.preferredEvent.trim()}</p>`;
  }
  if (params.promoterNotes?.trim()) {
    body += `<p>Message from promoter: ${params.promoterNotes.trim()}</p>`;
  }
  ...
  sendEmailSafe({
    to: params.guestEmail,
    subject: `Promoter update — ${statusLabel}`,
    html: layout(`Hi ${params.guestName},`, body),
    text: `Your profile inquiry status is now ${statusLabel}.${params.promoterNotes ? ` Note: ${params.promoterNotes}` : ""}`,
  });
}
